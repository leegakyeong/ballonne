...
function App() {
  ...
  async function onSceneReady(scene: Scene) {
    ...
    letters.forEach((letter, i) => {
      ...
      // Bevel 생성 함수
      function createBeveledPath(shape, depth, bevelSize, bevelSegments) {
        const paths = [];
      ...
      // const backBevelpaths = beveledPaths.map(p => p.add(new BABYLON.Vector3(0, 0, depth))); // Z 방향으로 이동

      // Extrusion 생성
      const bevelMesh = BABYLON.MeshBuilder.CreateRibbon(
        "beveledExtrusion",
        { pathArray: beveledPaths, closeArray: true, closePath: true, sideOrientation: Mesh.DOUBLESIDE },
        scene
      );

      // 앞면 닫기
      // const frontBuilder = new BABYLON.PolygonMeshBuilder("front", beveledPaths.map(p => new BABYLON.Vector3(p.x, p.y, p.z)), scene);
      ...
      // // 뒷면 닫기
      // const backBuilder = new BABYLON.PolygonMeshBuilder("back", backBevelpaths.map(p => new BABYLON.Vector3(p.x, p.y, p.z)), scene);
